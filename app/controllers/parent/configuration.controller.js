...
exports.updateParent = (req, res) => {
    ...
    Parent.findByIdAndUpdate(
        ...
        .catch((err) => {
            ...
            return res.status(500).send({
                message: "Erreur lors de la mise à jour du Parent avec l'ID " +
                    req.params.parentId,
            });
        });
};
...
exports.deleteParent = (req, res) => {
    Parent.findByIdAndRemove(req.params.parentId)
        ...
        .catch((err) => {
            ...
            return res.status(500).send({
                message: "Impossible de supprimer le Parent avec l'ID " + req.params.parentId,
            });
        });
};
...
exports.disableParentAccount = (req, res) => {
    Parent.findByIdAndUpdate(
            req.params.parentId, { statut: "inactif" }, { new: true }
        )
        .then((parent) => {
            if (!parent) {
                return res.status(404).send({
        ...
        .catch((err) => {
            ...
            return res.status(500).send({
                message: "Erreur lors de la désactivation du compte Parent avec l'ID " +
                    req.params.parentId,
            });
        });
};
...
exports.enableParentAccount = (req, res) => {
    Parent.findByIdAndUpdate(
            req.params.parentId, { statut: "actif" }, { new: true }
        )
        .then((parent) => {
            if (!parent) {
                return res.status(404).send({
        ...
        .catch((err) => {
            ...
            return res.status(500).send({
                message: "Erreur lors de la réactivation du compte Parent avec l'ID " +
                    req.params.parentId,
            });
        });
};